    ...
    * Toward the Optimal Preconditioned Eigensolver: Locally Optimal
    ...
    if (eigen_solver == 'arpack' or eigen_solver != 'lobpcg' and
       (not sparse.isspmatrix(laplacian) or n_nodes < 5 * n_components)):
        ...
        try:
            ...
            largest, _ = eigsh(-1*laplacian, k=1, which='LM')
        except RuntimeError:
        ...
        X[:, 0] = dd.ravel()
        eigvals, diffusion_map = lobpcg(laplacian, X, M=M, tol=1.e-5,
                                  largest=False)
        largest = eigvals[-1]
    ...
    if eigen_solver == "lobpcg":
        ...
        if n_nodes < 5 * n_components + 1:
            ...
            eigvals, diffusion_map = eigh(laplacian)
            ...
            X = random_state.rand(laplacian.shape[0], n_components + 1)
            X[:, 0] = dd.ravel()
            eigvals, diffusion_map = lobpcg(laplacian, X, tol=1e-15,
    ...
    if drop_first:
        return embedding[1:n_components].T, largest - eigvals[1:n_components]
    else:
        return embedding[:n_components].T, largest - eigvals[:n_components]
